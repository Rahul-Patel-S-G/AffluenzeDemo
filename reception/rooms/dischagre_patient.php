...
$sql_beds="SELECT distinct ba.room_id,rc.room_name, sum(number_of_days) as quantity,rc.price, rc.price * sum(number_of_days) as totalprice FROM beds_allotment ba,room_category rc where ba.patient_id='{$patient_id}' and ba.room_id=rc.room_id and ba.flag=0 GROUP BY ba.room_id";
