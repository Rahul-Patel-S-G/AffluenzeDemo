...
if($paymentmode=="Both")
{
$cashamnt=$data->display_pay_details->devidecash;
if(!isset($cashamnt))
$cashamnt=0;
...
$insert_makepayment="insert into make_payment(patient_id,date,time,cashamt,cardamt,totalamt,paymentmode,user_id) value('{$patient_id}','{$currdate}','{$currtime}','{$cashamnt}','{$cardamnt}','{$total}','{$paymentmode}','{$user_id}')";
...
$update_makepayment="update make_payment set totalamt='{$total}' where bill_no = '{$bill_no}' ";
