...
if($flag=="test")
{
    ...
    //  foreach($data->testnames as $key1=>$val1)
       //{
			$teststaken="insert into op_lab_tests_taken(description,paymentmode,cashamt,cardamt,amount,date,time,patient_id,user_id) values('{$flag}','{$paymentmode}','{$cashamt}','{$cardamt}','{$amount}','{$date}','{$time}','{$patient_id}','{$user_id}')";
