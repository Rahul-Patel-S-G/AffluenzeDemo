...
if($flag=="package")
{
    ...
    //  foreach($data->testnames as $key1=>$val1)
       //{
			$packagetaken="insert into op_lab_package_taken(description,patient_id,package_id,date,time,paymentmode,cashamt,cardamt,amount,offer,totalamount,user_id) values('{$description}','{$patient_id}','{$package_id}','{$date}','{$time}','{$paymentmode}','{$cashamt}','{$cardamt}','{$amount}','{$offer}','{$offeramount}','{$user_id}')";
